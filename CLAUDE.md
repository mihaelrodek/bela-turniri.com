...
```
Port scheme: project "85" — backend 8085, Vite 5185, game 8285, Postgres 5485, MinIO 9085/9185. Keep them in sync across `application.properties`, `docker-compose.yaml`, `vite.config.ts`, `StartupSanityCheck.java` and game Dockerfile when changing.
...
Three locales: Croatian (default), Slovenian and English. Frontend engine is `src/i18n/index.ts` — no library: a module-level locale readable from non-React code, `useTranslation()` on `useSyncExternalStore`, `{placeholder}` interpolation, keys `"namespace.key"` split at the first dot, dictionaries one level deep in `src/i18n/{hr,sl,en}/{common,tournament,profile,pages,forms,whatsNew,admin,legal,game,blok}.ts`. `hr` is the source of truth (`type Dictionary = typeof hr`) and each `sl`/`en` namespace is typed against its hr counterpart, so a missing key in either is a compile error. Everything but `hr` is a dynamic `import()` (`localeLoaders`), and `admin`/`legal`/`game`/`blok` are route-scoped chunks per locale (`namespaceLoaders`, typed `Record<Locale, …>` so a half-registered language will not compile) — adding a language means a `localeLoaders` entry, four `namespaceLoaders` entries, a `LOCALE_LABELS` entry and a `pluralCategory` branch; `Locale`, `LOCALES`, storage and browser detection all derive from `localeLoaders`. **Counts must go through `usePlural()` / `tPlural()`** with `.one`/`.two`/`.few`/`.other` leaves: Croatian has three CLDR categories, Slovenian four because it kept the dual, English only two (`n === 1` → `one`, else `other` — its `.two`/`.few` leaves exist to satisfy the type and just repeat `.other`). `utils/format.ts` follows the active locale for dates and currency, and exports `intlTag()` as the single locale→BCP-47 map (`hr-HR`/`sl-SI`/`en-GB`; English is en-**GB** so the day-month order, 24-hour clock and Monday-first week survive translation, and currency stays EUR everywhere). Native mirrors it too: `android/app/src/main/res/values-{sl,en}/strings.xml` plus a matching `<locale>` in `res/xml/locales_config.xml`.

Backend mirrors it: `filters/LocaleRequestFilter` reads `X-Locale`, `services/MessageService` serves `resources/i18n/messages_{hr,sl,en}.properties` (identical key sets, UTF-8 via an explicit reader). Use `messages.t(key)` for the caller, but **`messages.t(locale, key)` with the recipient's stored `UserProfile.locale` for anything composed for someone else** — push notifications resolve the locale on the request thread and carry the finished payload into the background sender. Bare machine codes the SPA switches on (`UNPAID_REQUIRED`, `ALREADY_FINISHED`, …) are never translated. SEO preview controllers stay Croatian-only on purpose.
...
### Frontend structure
Providers in `main.tsx`: Chakra (`system.ts`, semantic tokens with `_light`/`_dark` twins and a `brand` palette — use tokens like `bg.panel`, `fg.muted`, `border.subtle`, never raw `gray.*` fills) → color mode (`color-mode.tsx`, synced to profile via `ThemeSync`) → TanStack Query `PersistQueryClientProvider` (`queryClient.ts`: `qk` key registry, 30 s staleTime, localStorage persistence of public queries only, bump `CACHE_BUSTER` when a cached DTO shape changes) → `AuthProvider` → router, all inside `ErrorBoundary`. Data fetching: `api/*.ts` stay plain async functions; pages use `useQuery(qk.x)`; own profile comes from `hooks/useMyProfile.ts` (invalidate `qk.profile` after profile mutations). TournamentsPage prefetches details on card hover/pointerdown. Heavy pages are `React.lazy` via `lazyWithReload` in `App.tsx`; `vite.config.ts` splits `vendor`, `vendor-map` (leaflet), `vendor-firebase` — react-leaflet must stay in `vendor` (init-order crash). `public/sw.js` is network-first for navigations and public `/api` GETs (never authenticated ones), and cache-first for `/assets/*`, which it precaches from `/precache-manifest.json` — written at build time by the `bela-precache-manifest` plugin in `vite.config.ts` (app shell + the `/blok` route chunk, static-import closure only) so an installed PWA opened offline still reaches the scorepad; the worker re-reads that manifest on every load because its own bytes never change (`SwUpdateToast` posts `bela:precache`). A route whose chunk is missing offline throws `OFFLINE_CHUNK_ERROR` from `lazyWithReload` and `ErrorBoundary` shows `OfflineNotice` — never a blanket offline screen, which would kill the API snapshot cache's whole point. `TournamentDetailsPage` polls rounds/pairs via `hooks/usePolling.ts` while STARTED. Use `components/ConfirmDialog.tsx` instead of `confirm()`, toaster instead of `alert()`. **No user-facing string is hard-coded** — everything goes through the dictionaries, so a new label means a new key in both `hr` and `sl`. Shared helpers: `utils/format.ts`, `utils/phone.ts`, `utils/ics.ts`, `components/EmptyState.tsx`, `SectionCard.tsx`, `SuffixInput.tsx`. Routes in `App.tsx`; `RequireAuth` guards protected pages. `toaster.ts` is the single shared toaster. `hooks/useDocumentHead.ts` sets per-page title/meta for SEO. PWA install prompt and push subscription live in `components/FirstRunInstallPrompt.tsx`, `InstallAppButton.tsx`, `PushBootstrap.tsx`, `hooks/usePushSubscription.ts`. `PageTour.tsx` + `tourSteps.ts` drive react-joyride onboarding.

### Game (bela online)
Standalone Node.js WebSocket server (`game/packages/server`) listens on **8285**, with public WebSocket path `/ws/game` rewritten by Caddy in prod and by Vite proxy in dev (both to `ws://localhost:8285`). Server verifies Firebase auth tokens just like the backend and maintains game lobbies, tables, seats, turn timers, reconnection, and bot players. See `game/README.md` for full architecture — the server is the single source of truth, frontend UI (`frontend/src/game/`) is read-only client views (table render, hand UI, animations, toast notifications). Engine logic (`game/packages/engine`) is deterministic and zero-dependency; frontend imports it directly from source via Vite alias (`@bela/engine` → `game/packages/engine/src/index.ts`) for client-side card logic (hand analysis, bid helpers). `./ops/deploy.sh` includes the game service in build + rollback logic. Ships **off by default** in prod behind a kill switch — `ops/game-flag/ENABLED` (bind-mounted into `edge`, same pattern as the maintenance flag) drives Caddy's `/game-status.json`, which `frontend/src/game/hooks/useGameEnabled.ts` polls to hide the nav link and gate the `/igra` routes; flip it with `./ops/toggle-game.sh on|off|status` — no rebuild needed.

## Gotchas
- Adding a UI string means adding it to **all three** of `hr`, `sl` and `en`; a missing Slovenian or English key fails `tsc` by design.
- Counts need a plural family, never a ternary — Slovenian's dual makes "2 turnirja" a different form from "3 turnirji". English fills `.two`/`.few` with the same text as `.other`; it never selects them.
- Never branch on the locale with a ternary (`locale === "sl" ? … : …`) — with three languages that silently lumps the third in with Croatian. Use a `Record<Locale, …>` or `intlTag()`.
