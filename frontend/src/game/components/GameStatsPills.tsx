...
import { STAT_TARGET_SCORES, overallRecord, targetRecord, winPercent } from "../util/gameStats"
 ...
 *  variant relies on `SimpleGrid` to divide its own width four ways evenly,
 *  a fixed tile width there would just leave a gap or overflow at 340px. */
function StatTile({ label, record, w }: { label: string; record: GameStatRecord; w?: string }) {
    return (
        <VStack gap="0" px="1" py="1" rounded="lg" bg="bg.subtle" borderWidth="1px" borderColor="border.subtle" minW="0" w={w} flexShrink={0}>
            <Text fontSize="2xs" color="fg.muted" fontWeight="semibold" lineHeight="shorter" whiteSpace="nowrap">{label}</Text>
            <Text fontSize="2xs" fontWeight="semibold" lineHeight="shorter" fontVariantNumeric="tabular-nums" whiteSpace="nowrap">
 ...
 *  discipline (501/701/1001), same numbers and rounding RoomPanel's seat
 ...
 *  4-tile content stay in exactly one place (2026-09-20, user request: put
 ...
 *   - `"grid"` (default): a 4-column grid that fills its container, up to
 *     340/420px — the phone header's own row.
 *   - `"row"`: four fixed-width (~112px) tiles side by side, meant to sit
        ...
        return (
            <HStack gap="1.5" flexShrink={0}>
                ...
                {STAT_TARGET_SCORES.map((target) => (
                    <StatTile key={target} label={target} record={targetRecord(stats, target)} w="112px" />
                ))}
            </HStack>
        )
    }
    return (
        <SimpleGrid columns={4} gap="1.5" w="full" maxW={{ base: "340px", md: "420px" }}>
            <StatTile label={overallLabel} record={overallRecord(stats)} />
            {STAT_TARGET_SCORES.map((target) => (
                <StatTile key={target} label={target} record={targetRecord(stats, target)} />
            ))}
        </SimpleGrid>
    )
}
