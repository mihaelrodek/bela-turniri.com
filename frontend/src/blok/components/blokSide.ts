/* ──────────────────────────────────────────────────────────────────────────
   ...
   "MI" is the `green` palette (teal) and "ONI" the `red` one. THEME.md asked
   for brand green / tan (2026-09-21 rollout), but the dark theme read too
   loud that way and the owner preferred the earlier green/red, so the pair
   stays as it was. Four components need to agree on that (the header totals,
   the deal rows, the winner card, the two bottom buttons). Hard-coding a
   palette at each of them is how the app ends up with a green "MI" button
   above a teal "MI" score, so the mapping lives here and every consumer sets
   ...
   Why palette-relative tokens and not literal ramp steps: `green` and `red`
   ship AA-checked `fg`/`solid`/`contrast` pairings for the light and the dark
   theme. Writing a ramp step here would opt out of all of that and break the
   moment the theme moves.
